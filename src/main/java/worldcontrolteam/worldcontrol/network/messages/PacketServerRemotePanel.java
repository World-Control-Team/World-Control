    ...
    public static class Handler implements IMessageHandler<PacketServerRemotePanel, IMessage> {
        ...
        public IMessage onMessage(PacketServerRemotePanel message, MessageContext ctx) {
            ...
            if(stack != null && stack.getItem() instanceof ItemBaseCard){
                ...
                CardState state = card.update(ctx.getServerHandler().playerEntity.mcServer.worldServers[0], stack);
        ...
        /*private ItemStack processCard(ItemStack card, int upgradeCountRange, int slot) {
            if (card == null) {
                return null;
            }
            Item item = card.getItem();
            if (item instanceof IProviderCard) {
                ...
                if (needUpdate) {
                    ...
                    state = ((IProviderCard) item).update(FMLServerHandler.instance().getServer().worldServers[0], card);
