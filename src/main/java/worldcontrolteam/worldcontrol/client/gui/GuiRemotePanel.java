...
public class GuiRemotePanel extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (inv.getStackInSlot(0) != ItemStack.EMPTY && itemInv.getStackInSlot(0) != ItemStack.EMPTY && itemInv.getStackInSlot(0).hasCapability(WCCapabilities.CARD_HOLDER, null)) {
            Card card = CardUtils.getNullableCard(inv.getStackInSlot(0));
            ...
            if (true)
                if (itemInv.getStackInSlot(0).hasTagCompound())
                    joinedData = card.getData(new LinkedList<>(), 0, true, new int[0]);
